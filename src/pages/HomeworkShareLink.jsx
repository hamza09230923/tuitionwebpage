...
import { createHomework } from '../api/functionsClient'

const STORAGE_KEY = 'pendingHomework'

function HomeworkShareLink() {
  ...
  useEffect(() => {
    const stateHomework = location.state?.pendingHomework
    if (stateHomework) {
      sessionStorage.setItem(STORAGE_KEY, JSON.stringify(stateHomework))
      setPending(stateHomework)
      return
    }
  ...
  const handleSave = async (e) => {
    ...
    try {
      await createHomework({
        ...
        attachmentName: pending.attachmentName || pending.fileName || null,
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-3">Homework Details</h2>
        ...
        <form onSubmit={handleSave} className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <div className="bg-gray-50 border border-gray-200 rounded-md p-3 text-sm text-gray-700 space-y-2 mb-4">
            ...
            <p>3) Paste the link below and click Save Homework.</p>
          </div>
...
export default HomeworkShareLink
